...
export async function POST(req) {
  ...
  try {
    ...
    // Let's create the response stream and save to DB in parallel
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // --- Database operations moved outside the stream ---
          // This controller only handles sending data to the client.
    ...
    // We can't await the full stream here, but we can process the DB logic after the streaming starts.
    // A better approach is to read the full response, then save, then stream it back.
    // For a true stream-then-save, we need to read the stream twice which is complex.
    // Let's go with a simpler, more robust method: get full response, save, then stream.
    ...
    // Now, create a new stream from the fully generated text
